...
public class AnalogClockwork {
    ...
    // Add a property to hold the current time
    private ReadOnlyObjectWrapper<LocalDateTime> time = new ReadOnlyObjectWrapper<>(LocalDateTime.now());

    // Method to access the time property
    ...
    private void startHandTicking(Duration tickDuration, EventHandler<ActionEvent> onTick) {
        Timeline timeline = new Timeline(new KeyFrame(tickDuration, onTick));
